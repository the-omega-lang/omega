...
pub enum CompileError {
    ...
    /// A package root contains no modules at all. Reported here rather than
    /// left to surface later as an internal panic. Carries no module or span
    /// (there is no module to anchor to), so it renders headline-only like
...
impl CompileError {
    ...
    pub fn to_diagnostics(&self) -> Vec<Diagnostic> {
        match self {
            ...
            // No span: definition site vs. invocation site is ambiguous for a
            // spliced token stream, so this renders headline-only.
...
pub struct CompiledProgram {
    ...
    /// referenced (see `Driver::collect_extern_functions`). An extern
    /// module's items are scanned, never compiled, so codegen must declare
    /// each one itself as `Linkage::Import`, trusting the deterministic
    /// mangling to match the symbol the other `omgc` invocation produces.
