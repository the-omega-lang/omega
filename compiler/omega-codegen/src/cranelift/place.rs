                ...
                MirProjection::EnumDynamicField { index, r#type, .. } => {
                    ...
                    current = match current {
                        PlaceStorage::Values(values) => {
                            ...
                            let len = enum_type.dynamic_fields[*index]
                                .r#type
                                .cranelift_leaves(self)
                                .len();
                            PlaceStorage::Values(values[start..start + len].to_vec())
                        }
                        PlaceStorage::Slot { slot, offset } => PlaceStorage::Slot {
    ...
    pub(super) fn place_storage_address(
        ...
        match storage {
            ...
            PlaceStorage::Values(values) => {
                let size: u32 = values
                    .iter()
                    .map(|v| builder.func.dfg.value_type(*v).bytes())
                    .sum();
                let slot = builder.create_sized_stack_slot(StackSlotData::new(
                    StackSlotKind::ExplicitSlot,
                    size,
            ...
            PlaceStorage::Slot { slot, offset } => {
                builder.ins().stack_addr(ptr_type, *slot, *offset as i32)
            }
        ...
        for leaf in r#type.cranelift_leaves(self) {
            let value = match storage {
                PlaceStorage::Slot { slot, offset } => {
                    builder
                        .ins()
                        .stack_load(leaf, *slot, (*offset + rel_offset) as i32)
                }
                PlaceStorage::Address { base, offset } => {
                    builder
                        .ins()
                        .load(leaf, MemFlags::new(), *base, (*offset + rel_offset) as i32)
                }
        ...
        for value in values {
            ...
            match storage {
                ...
                PlaceStorage::Slot { slot, offset } => {
                    builder
                        .ins()
                        .stack_store(*value, *slot, (*offset + rel_offset) as i32);
                }
                PlaceStorage::Address { base, offset } => {
                    builder.ins().store(
                        MemFlags::new(),
