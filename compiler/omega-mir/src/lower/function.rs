...
use crate::body::{
    ...
    MirSlice, MirSpecCoerce, MirStructLiteral, MirTerminator, MirUnionConstruct, MirUnsizeSlice,
};
...
impl FunctionLowerer {
    ...
    pub(super) fn lower_expr(&mut self, node: CheckedExprNode) -> MirExprNode {
        ...
        match kind {
            ...
            CheckedExpr::UnsizeSlice(unsize) => {
                let base = Box::new(self.lower_expr(*unsize.base));
                let kind = MirExpr::UnsizeSlice(MirUnsizeSlice { base, len: unsize.len });
                MirExprNode { id, span, r#type, kind }
            }
...
fn collect_defer_ids_expr(expr: &CheckedExprNode, out: &mut Vec<(HirId, Span)>) {
    match &expr.kind {
        ...
        CheckedExpr::UnsizeSlice(unsize) => collect_defer_ids_expr(&unsize.base, out),
        CheckedExpr::DynamicCall(call) => {
