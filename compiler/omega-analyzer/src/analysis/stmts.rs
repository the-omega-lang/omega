...
impl<'r> Analyzer<'r> {
    ...
    fn analyze_for_in(&mut self, f: &HirForIn) -> Option<Vec<CheckedStmt>> {
        // A range-driven loop (`for i in a..<b { }`) is intercepted here,
        // before `f.iterator` is ever analyzed the ordinary way -- a
        // standalone range has no real value (`HirExpr::Range`'s own doc
        // comment) to feed through `classify_for_in_source`'s `ToIterator`/
        // `Iterator` machinery at all. See `analyze_for_in_range`.
        if let HirExpr::Range(range) = &f.iterator.expr {
            return self.analyze_for_in_range(f, range);
        }
    ...
    /// `for <mut>? binding in a..<b { }` / `a..=b` / `a.. { }` -- desugars
    /// directly into the classic three-clause `for`'s own `CheckedFor`
    /// shape (see `analyze_for`), never the iterator-protocol `while` +
    /// `match` above: there is no first-class `Range` value in this
    /// language (ranges are a purely structural, compile-time concept
    /// everywhere else too -- see `omega_parser::ast::range::RangeExpr`'s
    /// doc comment), so a range here is consumed directly at the
    /// `HirExpr::Range` shape, never a real value flowing through
    /// `ToIterator`/`Iterator`.
    ///
    /// A private counter (`$i`, never user-visible) drives the loop; the
    /// user's own `binding` is a fresh copy taken from it each iteration
    /// (`<mut>? binding := $i;`), exactly like the iterator-protocol
    /// desugaring's own `binding := $next.value` -- decoupled from the
    /// counter's own mutability needs, which this desugaring alone
    /// controls.
    ///
    /// Exclusive ends (`a..<b`) use the ordinary, naive `while $i < b {
    /// ...; $i += 1; }` shape -- provably safe from overflow, since `b` is
    /// itself always representable, so the last value `$i` is ever
    /// incremented from is `b - 1`. Inclusive ends (`a..=b`, and `a..`'s
    /// implicit `b = domain.max`) can't safely use that shape: if `b`
    /// happens to be the element type's own actual maximum (guaranteed
    /// for an open-ended `a..`, possible for any user-written `a..=b`
    /// too), incrementing past the last iteration would overflow. Those
    /// use a `$more`-flag shape instead that never computes `b + 1` at
    /// all -- `$more` is flipped to `false` (rather than `$i` incremented)
    ...
    fn analyze_for_in_range(&mut self, f: &HirForIn, range: &HirRange) -> Option<Vec<CheckedStmt>> {
        ...
        let result = (|| {
            ...
            let checked_end = match &range.end {
                Some(e) => {
                    let checked = self.analyze_expr(e, Some(&element_type))?;
                    if checked.r#type != element_type {
                        self.error(
                            f.id,
                            e.span,
                            AnalysisErrorKind::ForLoopRangeBoundTypeMismatch {
                                expected: element_type.clone(),
                                found: checked.r#type,
                            },
                        );
                        return None;
                    }
                ...
                None => Self::number_literal(f.id, f.span, &element_type, domain.1),
            };
            ...
            let user_tail = match &f.body.tail {
                Some(t) => self.analyze_expr(t, None).map(Box::new),
                None => None,
            };
            self.loop_stack.pop();
            let scope = self.context.leave_scope();
