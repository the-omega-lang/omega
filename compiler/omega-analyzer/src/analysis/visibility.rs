...
impl<'r> Analyzer<'r> {
    ...
    /// resolver, across a trait boundary this `reveal_stack` can't reach
    /// directly, so it threads the same bypass decision (`!self.reveal_stack.
    ...
    /// bearing, for the identical `UnnecessaryReveal` warning.
    ///
    ...
    /// allowed -- if the ordinary rule would deny it but a `reveal` frame is
    ...
    /// The raw, `reveal`-blind visibility decision -- `check_visibility`'s
    /// own "allowed" computation, factored out so a caller that must *not*
    /// consult the ambient `reveal_stack` at all (currently only
    ...
    /// something a call-site `reveal` can expand) can reuse the exact same
    ...
    /// hidden *member*'s scope is narrower than a hidden *item*'s: "a
    /// hidden field/method cannot be accessed outside of the struct
    /// definition," not merely outside its declaring module. `owner_id` is
    /// the declaring struct/union/enum's own stable identity
    /// (`ResolvedStructType::id` and friends); `Hidden` is allowed only
    ...
    /// touch a hidden field or call a hidden method of any type,
    /// including one declared in its own module, without `reveal` --
    ...
    /// least one was present and the first non-`Reveal` node reached -- a
    /// no-op (`(false, expr)`) when there's no `Reveal` wrapper at all, the
    ...
    /// pattern-matching `HirExpr::Place`, since `reveal` is a real, generic
    /// wrapper node -- never folded into `HirPlace` itself (see
    /// `HirExpr::Reveal`'s doc comment) -- so a `reveal`-wrapped place would
    /// otherwise look like "not a place at all" to these checks.
    ///
    /// Critically, `reveal` here wraps only the *sub-position*
    /// (`assignment.target`, `addr.base`, ...), not the enclosing expression
    /// (`reveal a.b = c;` parses as `Assignment { target: Reveal(FieldAccess(a,
    /// b)), value: c }` -- `parse_assignment`'s own target is parsed via the
    /// same precedence descent that bottoms out at `parse_unary`, where
    /// `reveal` is recognized, so it binds to `a.b` alone, never spanning
    /// the `=` and beyond). That means `analyze_expr`'s own `HirExpr::Reveal`
    /// arm -- which only ever sees a `Reveal` node when it's the *outermost*
    ...
    /// `with_reveal_bypass(was_reveal, ...)` using the `bool` returned here,
    ...
    /// `analyze_expr`'s own `HirExpr::Reveal` arm (see `strip_reveal`'s doc
    /// comment for why that arm doesn't fire on its own here). Pops the
    /// frame and emits `UnnecessaryReveal` (anchored at `node_id`/`span`,
    /// the *enclosing* expression's own -- there is no narrower span to
    /// anchor at, since the `Reveal` node itself was never assigned one of
