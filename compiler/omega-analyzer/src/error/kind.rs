...
pub enum AnalysisErrorKind {
    ...
    /// `&ptr[start..]`/`&ptr[start..<]` (an open-ended range) on a raw
    /// pointer base -- unlike `SizedArray`/`Slice`/`Str`, a bare pointer
    /// carries no length anywhere to default a missing end to.
    MissingSliceEnd,
    /// `&comp_binding[range]`/`&comp_binding.tail_field[range]` where the
    /// base is a raw pointer or a struct's own `[T]` tail marker -- neither
    /// has an established `comp`-time const-promotion story (see
    /// `Analyzer::analyze_slice`'s own comment on this); narrow and likely
    /// never hit in practice, but rejected explicitly rather than silently
    /// mishandled.
