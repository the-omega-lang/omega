...
A value and a call that reach the same declaration with the same generic arguments share one instantiation, and therefore one address, however each spelled the selection. Two *different* declarations are two instantiations and two addresses even at the same generic arguments, and even where they produce the same signature -- which is exactly what a selector can arrange.
