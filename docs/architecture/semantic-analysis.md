...
A written generic argument list on a function is an `ExprGenericArg` list rather than a plain `GenericArg` list: an entry may carry a **bound selector** naming the declared bound set the declaration it selects must have. The distinction is syntactic and stops at the analyzer -- `analysis/calls/selector.rs` resolves a written list into `WrittenGenerics` (positional bindings, with a `spec ...` entry leaving its slot open, plus the canonical bound keys it requires), and every position that does not accept a selector converts back to `GenericArg` and rejects one there.
