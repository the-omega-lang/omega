...
A generic *member or static* declaration is not an item query, because it belongs to an owner instantiation rather than to a module. It therefore has no signature to collect and is left out of its owner's resolved methods entirely; `analysis/items` skips it in both the signature and the body pass. Every call site instead asks the resolver for the declaration's raw template (`generic_method_template`), infers its arguments against the owner substitution the receiver or path already fixed, and asks for the instantiation (`instantiate_generic_method`), which is an ordinary `ResolvedMethod` from that point on. A position that determines nothing -- naming a generic declaration with neither an expected function type nor written arguments -- is rejected where it is written.

An *uncalled* reference is the second context that can determine those arguments, and it runs through one shared selector (`analysis/calls/value.rs`) rather than through call resolution. Candidate discovery stays driver-owned: `function_value_candidates` and `generic_method_candidates` extend the call-site candidate queries with the lone declarations a call reaches by its own path, since a value reference has no such path. The selector then binds any written prefix, infers the rest from the expected signature through the same `TypePattern` machinery calls use, and requires a recursively exact match (`TypePattern::identical`) rather than the acceptance predicate a call's conversion cost pays for. Ranking is the call's own preference rule, and only the winner is instantiated, through the same identity/cache path -- so a value and a call that select the same declaration with the same arguments share one monomorphization.

### Bound selection, preparation, and proof

A written generic argument list on a function is an `ExprGenericArg` list rather than a plain `GenericArg` list: an entry may carry a **bound selector** naming the declared bound set the declaration it selects must have. The distinction is syntactic and stops at the analyzer, and every position that does not accept a selector converts back to `GenericArg` and rejects one there.

`analysis/calls/selector.rs` reads such a list in two stages, because the two halves have different owners.

*Caller validation* (`validate_written_generics` -> `ValidatedGenerics`) settles everything that is true of the written list regardless of what the name offers: each selector's spec names, their visibility, the bounds the aliases in them declare, and the concrete type a typed selector fixes. It runs **once per call or value reference, with diagnostics on**, before any declaration is considered. An invalid selector is therefore a terminal error for that call rather than a reason to keep trying candidates, and adding, hiding, or reordering overloads can neither make one acceptable nor report it twice. An alias's own bounds are checked at the spelling that wrote them, before conjunction expansion erases the wrapper that declares them; `check_alias_generic_bounds` reports whether they held, because the expansion resolves either way and having reported the failure is not enough to stop a caller that would otherwise use it.

*Candidate binding* (`bind_written_generics` -> `WrittenGenerics`) applies that validated list positionally to one declaration: parameter kind, arity, how a plain argument reads against the slot it lands on, retained inference holes (a `_` or `_ : A` entry leaves its slot open), and the canonical bound keys each position requires. Only this stage is speculative, and it resolves no selector name and rechecks no alias constraint.

Every route shares both stages: `resolve_overload_candidates`, `select_function_value`, and the singleton paths through `resolve_written_generics`. Selection with or without an expected function type therefore sees the same validated selector data.

Deciding whether a candidate applies is a driver query, `ModuleResolver::prepare_generic_call`. Given a declaration -- by candidate identity, by absolute path, or by owner and name -- and the bindings a call site has, it completes the remaining arguments from the declaration's own defaults, reports the bound set each generic parameter then declares, and says whether those bounds are proved. It deliberately stops short of `ensure_item_at`: preparing a candidate that goes on to lose leaves no instantiation, no signature, and no analyzed body behind. Resolving a default a candidate needs to reach its bounds is the one thing preparation may do for a loser.

Both sides of a selector comparison go through `Analyzer::bound_key`, so a declared bound and a written one are canonicalized identically: aliases expanded, the spec resolved to its declaration identity, and the spec's own declared defaults applied to its arguments. An unmet alias-owned bound makes it no key at all rather than a reported-but-usable one. `check_generic_bound` uses the same key, which is what keeps a bound's spelling at the declaration and at the use site from disagreeing about which conformance would satisfy it. A bound is satisfied only by an actual conformance witness (`conformance_for`); the method-matching fallback in `type_implements_spec` remains for dynamic-shape questions and now only names what a failing type is missing.

`generics::compare_bound_sets` is still the ordering for declaration duplicates and blanket-conformance precedence. Function selection no longer uses it: bound strength does not rank declarations, and the caller's own written plain-type positions do (`WrittenGenerics::dominates`).
...
Expression analysis receives its context as an `Expected` (`analysis/expected.rs`): nothing, an exact `ResolvedType`, or a `TypePattern` whose `Parameter`s are the `_` holes of a binding annotation. A pattern is built by the same `overload_type_pattern` the overload engine uses, with each hole rewritten to a synthesized `$Hole{n}` parameter (`analysis/items/holes.rs`). Only the sites that *choose* something from the expected type read a pattern -- array literals and `&[...]`, generic struct/enum literal inference, generic-call seeding (`unify_generic_pattern`, which binds a callee parameter only from the pattern's known parts), and the pass-through of blocks, `if`/`match` arms, `comp`, and `reveal`; every other site takes `Expected::exact()` and sees no expectation. The initializer is analyzed once; the holes are then read back from its checked type with `TypePattern::infer`, and the annotation is re-resolved as if the solved type had been written. There is no probe pass, so an annotation never materializes an instantiation its initializer does not use.
...
A `CheckedMatch` also records what reaching none of its arms *means*, in `CheckedMatchRemainder`. A value match's arms partition the scrutinee's whole domain, so its remainder is genuinely unrepresentable; an enum match's arms cover every declared variant, so its remainder is a tag the declaration gave no meaning, which is a violated invariant a running program can still be in. An enum `else` is analyzed into a final arm conditioned on the variants the other arms left rather than kept as a fallthrough — a dead one is reported as unreachable code and dropped — so the two cases stay distinguishable without MIR re-deriving the scrutinee's kind. `runtime_checks` uses that distinction, and the resolved `core::panic` support the driver binds to it, to decide which bodies carry a `FunctionRuntimeChecks`; see [`mir-and-codegen.md`](mir-and-codegen.md#runtime-checks).

The try operator follows that rule rather than being desugared. `CheckedExpr::Try` keeps the user-written `?` as one node, with its own operator span, and stores the propagation facts the analyzer resolved: which canonical fallible family the operand is, the source success/failure variant indices, tag and payload field indices, the enclosing fallible return type, the destination failure variant/field, and — for `Result` — the `E` -> `F` conversion. It is deliberately **not** replaced with a synthetic `CheckedMatch`/temporary/`CheckedStmt::Return` tree: a checked-tree consumer such as an LSP must still be able to see that a try operator existed and inspect its operand, success, and error types without reconstructing an operation the analyzer already understood.

That conversion is recorded as a `CheckedCoercion`: an ordered list of `CheckedCoercionStep`s (anonymous-enum member projection/injection/widening, spec-object coercion) decided once by `plan_coercion`. `coerce_to_expected` applies such a plan to a value it owns; `CheckedTry` stores one for a payload that does not exist yet. Downstream consumers replay the steps and never re-ask whether the conversion is legal.
...
A function with generic parameters of its own additionally carries a `TemplateDescriptor` (`template.rs`): the structural identity of the **declaration** it was instantiated from, taken before its own parameters were substituted. Bound selection can reach two declarations of one name at the same concrete arguments, so the instantiation's path, arguments and signature no longer say which body a symbol refers to; this is the missing component, and [`symbol-mangling.md`](symbol-mangling.md#declaration-identity) owns what it contains and why. It is derived from the same `overload_template` patterns selection already uses, which is why those patterns carry each spec's declared path (`SpecPattern`) rather than only a local `HirId`: a linker name needs an identity two processes agree on. Aggregate fields are represented by `CheckedField`; function parameters by `CheckedParam`. The distinction exists in HIR (`HirField` / `HirParam`) and resolved semantic data (`ResolvedField`) as well, so field visibility/identity is not smuggled through a parameter-shaped tuple.
